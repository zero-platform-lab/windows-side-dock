...
//! 画面の右端を確保する。最大化したウィンドウは確保した分の手前で止まる。
...
use windows_sys::Win32::UI::Shell::{
    SHAppBarMessage, ABE_RIGHT, ABM_NEW, ABM_QUERYPOS, ABM_REMOVE, ABM_SETPOS, APPBARDATA,
};
...
fn appbar_data(window: HWND) -> APPBARDATA {
    APPBARDATA {
...
/// `window` のあるモニターの右端を `width` ピクセル確保し、Dockを置ける範囲を返す。
/// 横はWindowsが決めた確保範囲、縦は作業領域（タスクバーを除いた範囲）。
pub(crate) fn reserve_right_edge(window: HWND, width: i32) -> Option<egui::Rect> {
    if window.is_null() {
        return None;
    }
